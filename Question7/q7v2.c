/* 
Synchronization between the thread "hello" and thread "world" to show
the correct ordering of their display
*/
...
int main(){
	...
	pthread_join(hellotd,NULL);			// wait for hello thread (not necessary, just to demonstrate behavior in non-syschronization case)
	printf("\n");
	return 0;
}
...
void* world(void* beforeId){
	...
	pthread_join(*before,NULL);			// wait for the completion of hello thread.
	printf("world");
	pthread_exit(NULL);
}
